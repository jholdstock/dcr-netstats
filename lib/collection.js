...
Collection.prototype.addBlock = function(id, stats, callback)
{
	...
	else
	{
		...
		else
		{
			...
			if(block.block.height > this._highestBlock)
			{
				this._highestBlock = block.block.height;
			}
...
Collection.prototype.updatePeers = function(peers, callback) {
	if (peers && peers.length) {
		peers = peers.map(function (val) {
	...
	return callback(new Error('Peers array is empty'), null);
}

Collection.prototype.updateLocked = function(locked, callback) {
		this._blockchain.updateLocked(locked, callback);
}

Collection.prototype.updateSupply = function(supply, callback) {
		this._blockchain.updateSupply(supply, callback);
}

Collection.prototype.updateMiningInfo = function(stats, callback) {
...
Collection.prototype.peers = function()
{
	return this._peers;
}
...
Collection.prototype.getBestBlockFromItems = function()
{
	return Math.max(this._blockchain.bestBlockNumber(), _.result(_.max(this._items, function(item) {
		// return ( !item.trusted ? 0 : item.stats.block.height );
		return ( item.stats.block.height );
